...
function getMsg(num) {
    return 'Must use ' + num + ' spaces for indents.';
}
        ...
        for (var j = 0; j < selectorLen; j++) {
            ...
            for (var m = 0, elementsLen = elements.length; m < elementsLen; m++) {
                ...
                if (spacesNum === 0) {
                    ...
                    reg = new RegExp('^\\s{' + spacesNum + '}');
                }

                if (!reg.test(lineContent))  {
                    me.invalidList.push({
                        uniqueFlag: ruleName + lineNum,
                        ruleName: ruleName,
                        line: lineNum,
                        message: '`'
                            + lineContent
...
function dealPropertyIndent(realVal, ruleName) {
    var me = this;
    if (realVal.type !== 'Comment' && realVal.selectors) {
        var lineNum = util.getLine(realVal.index, me.fileData);
        var lineContent = util.getLineContent(lineNum, me.fileData, true);
        var spacesNum = (realVal.selectors[0].selectorLevel + 1) * 4;
        var reg;
