...
var errorMsg = ''
    + chalk.grey(''
        + 'When multiple selectors share a statement block,'
        + ' each selector statement must be per line.'
    );
 ...
 */
function analyzeSelectors(realVal) {
    ...
    // console.log(require('util').inspect(selectors, {showHidden: true, depth: null}), 'completecompletecomplete'.green);
    ...
    selectors.forEach(
        function (selector) {
            ...
            var tmpIndex = 0;
            // console.log(require('util').inspect(selector, {showHidden: true, depth: null}), 'completecompletecomplete'.green);
            // 第一个
            if (refLineNum === -1) {
            ...
            // console.log(util.getLine(element.index, me.fileData));
            // console.log(require('util').inspect(selector.elements, {showHidden: true, depth: null}), 'completecompletecomplete'.green);
...
module.exports = function (rule) {
    ...
    if (realVal.selectors) {
        ...
        // }
        if (length > 1) {
            analyzeSelectors.call(me, realVal);
        }
