...
function analyzeValue(value, name, unit, index) {
    debugger
    var me = this;
    if (Array.isArray(value)) {
        for(var i = 0; i < value.length; i++) {
            analyzeValue.call(me, value[i], value.name || name, value.unit || unit, value.index || index || 0);
            if ((i + 1) < value.length) {
        ...
        if (value.value || value.value === 0) {
            analyzeValue.call(me, value.value, value.name || name, value.unit || unit, value.index || index || 0);
        }
        else {
            // 说明 value 中有变量
            // if (typeof value === 'object' && value.name) {
            //     if (!me.realVal.refVars) {
            //         me.realVal.refVars = [];
            //     }
            //     console.warn(value, value.variable, 'lll'.red);
            ...
            me.realVal.index = index;
            me.realVal.value.push({
                value: value,
                unit: unit
            });
            console.log('========================');
            console.log(name, value, unit, 'bbbbbbbbbbbb'.green);
        }
    }
}
