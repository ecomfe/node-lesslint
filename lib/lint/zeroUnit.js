...
function analyzeValue(value, name, unit) {
    var me = this;
    if (Array.isArray(value)) {
        for(var i = 0; i < value.length; i++) {
            analyzeValue.call(me, value[i], value.name || name, value.unit || unit);
            if ((i + 1) < value.length) {
        ...
        if (value.value || value.value === 0) {
            analyzeValue.call(me, value.value, value.name || name, value.unit || unit);
        }
        else {
            me.realVal = {
                name: name,
            ...
            console.log('========================');
            console.log(name, value, unit, 222);
        }
    }
}
