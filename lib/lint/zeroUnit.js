...
module.exports = function (rule) {
    ...
    // console.log(realVal, '444444444444444444444444444');
    for (var i = 0, len = realVal.values.length; i < len; i++) {
                ...
                if (units.indexOf(v.unit.numerator[0]) !== -1) {
                    me.invalidList.push({
                        ...
                        message: '`'
                            // + name
                            // + ': '
                            // + (v.value + v.unit.numerator[0]).yellow
                            // + fileData.split('\r\n')[lineNum - 2].replace(/\s*/, '').yellow
                            + util.getLineContent(lineNum, fileData).yellow
                            + '`'
                            + ' '
                            + errorMsg
                    });
        ...
        // console.log(realVal.values[i], 1111, realVal);
    }

    // console.log(rule.realVal, '222222222222222222222222');
    // console.log(require('util').inspect(rule.realVal, { showHidden: true, depth: null }), '222222222222222222222222');
