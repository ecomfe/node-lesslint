...
exports.check = function (file, errors, done) {
    ...
    parser.parse(
        ...
        function (err, tree) {
            ...
            var invalidList = abstractVisitor.invalidList;

            if (invalidList.length) {
                for (var i = 0, len = invalidList.length; i < len; i++) {
                    errors.push({
                        index: invalidList[i].index,
                        message: invalidList[i].message
                    })
                }
            }
