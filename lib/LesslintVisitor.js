    ...
    fs.readdirSync(
        ...
        function (fileName) {
            ...
            if (me.detectConfigs[fileName]) {
                // 这个文件是肯定存在的，因为这里是以 rule 里的文件来循环的
                // ret[fileName] = require(path.join(__dirname, 'rule', fileName));
                ret[fileName] = {
                    configVal: me.detectConfigs[fileName],
                    func: require(path.join(__dirname, 'rule', fileName))
                };
            }
        }
    );
                ...
                if (rule.selectors && Array.isArray(rule.selectors)) {
                    ...
                    for (var i in me.detectConfigs) {
                        if (me.detectConfigs.hasOwnProperty(i)) {
                            me.detectConfigs[i].func.call(me, rule, i, me.detectConfigs[i].configVal);
                        }
                    }
                }
            }
 ...
 * 分析选择器的层级关系
 *
 * @param {Array} rule ast node
 * @param {Object} arr 缓存 selectors 的对象，也用于递归
 * @param {number} level 层级
 */
function analyzeSelectorLevel(rule, arr, level) {
    var t = {
        selectors: []
    };
    ...
    if (selectors) {
        ...
        selectors.forEach(function (selector) {
            if (arr.selectors.length !== 0) {
                if (isFlag) {
                    level++;
                    isFlag = false;
                }
                selector.parentSelector = arr.selectors[arr.selectors.length - 1];
                selector.selectorLevel = level;
            }
            t.selectors.push(selector);
        });
    }
    arr.selectors = arr.selectors.concat(t.selectors);

    var rs = rule.rules;
    if (rs) {
        for (var i = 0, len = rs.length; i < len; i++) {
            var r = rs[i];
            analyzeSelectorLevel(r, arr, level);
        }
    }
}
